...
class AttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        dot_product = (query @ key.transpose(1, 2)) / np.sqrt(self.embed_dim)
...
class SelfAttentionBlock(nn.Module):
    ...
    def forward(self, seq, mask):
        ...
        attn_output = self.dropout(self.self_attn(seq, seq, seq, mask))
        out = seq + attn_output
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, seq, cond):
        ...
        # Then add a residual connection to the original input, and finally apply normalization. #############################
        attn_output = self.dropout(self.cross_attn(seq, cond, cond, None))
        out = seq + attn_output
        out = self.norm(out)
        return out
...
class FeedForwardBlock(nn.Module):
    ...
    def forward(self, seq):
        ...
        # Then add a residual connection to the original input, and finally apply normalization. #############################
        mlp_output = self.dropout(self.mlp(seq))
        out = seq + mlp_output
        out = self.norm(out)
        return out
